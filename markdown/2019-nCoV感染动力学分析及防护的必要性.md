...
* R（Recovered），康复者，指因隔离或痊愈后有免疫力的人群。
...
假设一个地区总人数为N，I个感染者如不隔离，每人每天会碰到r个人，以概率$\beta$传染这r个人；潜伏者E会有概率$\alpha$成为感染者；同时感染者因治疗或免疫有$\gamma$的概率成为康复者R，可以建立传播动力学模型:
...
事实上，我们知道本次新型冠状病毒在潜伏期具有传染性，因此需要将潜伏者的传染性也要考虑进去。假设潜伏者以概率$\beta_2$将易感者S感染成新的潜伏者，潜伏者每天接触的人数为$r_2$，对模型进行改进，建立传染动力学模型：
...
```python
res = calculate(SEIR_II,seri,paraII,1)
plot_1graph(res)
```
